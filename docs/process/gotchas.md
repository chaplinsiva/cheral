...
- **Use scripts for stable logic, skills for evolving knowledge.** Static scripts are ideal when the rules are well-defined and unlikely to change. But when automation requires understanding things that change externally — evolving formats, shifting best practices, new API conventions — prefer a Antigravity skill over a script. Skills bring current understanding (and can web-search) on every run.
...
- **Navigation Blindness anti-pattern.** Creating disconnected screens without considering how the user transitions between them. Users complain that the framework does not consider "going from one page to another page". **Detection signal:** multiple pages/screens are built but no routing, navigation state, or back-buttons link them. **Fix:** explicitly plan screen-to-screen navigation and routing logic during the planning phase (`vteam-plan`). Dani must review the user flow, and Done Gate #8c must verify navigation state transitions.
